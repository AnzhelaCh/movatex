...
var CartDrawer = class extends Drawer {
  constructor() {
    ...
    this._onExampleAddedListener = this._onExampleAdded.bind(this);
...
var ProductCard = class extends HTMLElement {
  ...
  async _onSwatchHovered(event, target) {
    ...
    if (firstMatchingVariant && firstMatchingVariant.hasOwnProperty("featured_media")) {
  ...
  async _onSwatchChanged(event, target) {
    ...
    console.log(firstMatchingVariant, 'firstMatchingVariant');
    ...
    if (primaryMediaElement.src !== newPrimaryMediaElement.src) {
      ...
      const var_id = target.dataset.varid;
      if (var_id){
        this.querySelectorAll(`a[href^="${this.product["url"]}"`).forEach((link) => {
          const url = new URL(link.href);
          url.searchParams.set("variant", var_id);
          link.href = `${url.pathname}${url.search}${url.hash}`;
        });
      }
      const var_img = target.dataset.image;
      if (var_img){
        const primaryMediaElement = this.querySelector(".product-card__image--primary");
        primaryMediaElement.src = var_img;
        primaryMediaElement.srcset = var_img;
      }

      const second_img = target.dataset.secondimage;
      if (second_img){
        const secondMediaElm = this.querySelector(".product-card__image--secondary");
        secondMediaElm.src = second_img;
...
var PredictiveSearch = class extends HTMLElement {
  ...
  // note: previous version of predictive search. it doesnt support SKU so I had to replace it with old one
  // async _doPredictiveSearch() {
  //   await this._transitionToSlot("loading");
  //   const queryParams = `q=${this._queryInput.value}&section_id=${this.getAttribute("section-id")}&resources[limit]=10&resources[limit_scope]=each`;
...
reveal_fn2 = function() {const heading = this.querySelector('h2[data-reveal-on-scroll="true"]'), headingKeyframe = getHeadingKeyframe(heading);animate10(...headingKeyframe);
};
