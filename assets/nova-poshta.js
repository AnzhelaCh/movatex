function NovaPoshtaMain() {   
  ...
  const properties_all_cart  = {_provider: null, _country: null,_delivery_courier_address: null,_delivery_courier_street: null,_delivery_courier_house: null,_delivery_courier_flat: null,_delivery_street_Ref: null, _delivery_city: null,_delivery_city_name: null,_delivery_city_Area: null,_delivery_city_Ref: null,_delivery_warehouse: null,_delivery_warehouse_zip: null,_delivery_warehouse_name: null,_delivery_warehouse_address: null,_delivery_warehouse_CityRef: null,_delivery_warehouse_Number: null,_delivery_warehouse_Ref: null,_delivery_warehouse_SettlementRef: null,_delivery_warehouse_TypeOfWarehouse: null};
...
function nphtml() { 
  ...
  if (NP.widgetLocation == 'drawer' && document.querySelector('.drawer__footer.poshta-content') && NP.drawerType == 'theme') {
    NP.NovaPoshtaLoad = false;
    widget_load();
    console.log('loaded-only')
    return;
  }
  if (NP.widgetLocation == 'drawer' && NP.drawerType == 'rebuy')  { 
    document.querySelector(NP.selectorCheckoutButton).classList.add('hidden');
    var buttonHTML = '<div class="button checkout-button rebuy-button rebuy-cart__checkout-button block">Перейти до оплати</div>';
    document.querySelector('.rebuy-cart__flyout-actions').insertAdjacentHTML('afterbegin', buttonHTML);
    window.NP.selectorCheckoutButton = '.checkout-button';
  } 
  /*== loading ==*/ 
